...
public class ProcessRejectInvite {
	private ModelMap model;
	private String appId;

	public ProcessRejectInvite(ModelMap model, String appId) {
		this.model = model;
		this.appId = appId;
	}
	...
	public String execute() {
		...
		if (!app.getStatus().equals("invited")) {
			model.addAttribute("code", "403");
			model.addAttribute("message", "Forbidden, you are not allowed to do that!");
			return "error";
		}
		res = appDao.updateAppStatus(appId, "itv_rejected");
		if (res == null) {
			model.addAttribute("code", "500");
			model.addAttribute("message", "INTERNAL_SERVER_ERROR");
			return "error";
